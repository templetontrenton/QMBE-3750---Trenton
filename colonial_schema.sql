...
SELECT TripName
FROM trip
WHERE Type = 'Biking' AND Tripid IN (SELECT tripid FROM tripguides WHERE GuideNum = 'BR01');

#Question 13
SELECT firstname, lastname, tripname, type
FROM (SELECT customer.firstname, customer.lastname, reservation.tripid,trip.tripname, trip.type, customer.customernum
FROM customer
JOIN reservation
ON customer.customernum = reservation.customernum
JOIN trip
ON reservation.tripid = trip.tripid)
AS tripid
